...
double FromScale(unsigned long dwValue, int iHighBit, int iBitCount, int iSigned, double dScale) {
    ...
    int iRightBit = iHighBit - iBitCount + 1; // iRightBit - количество незначащих бит справа от числа
    if(iRightBit < 0) 
        return 0.0;     // размер числа iBitCount бит и его смещение вправо на iBitCount-1 бит
                        // не позволяют его разместить в N битах
    if(iRightBit > 0){              // при наличии справа от числа незначащих бит обнуляем эти биты
        dwValue >>= iRightBit;      // сдвинув число вправо
        dwValue <<= iRightBit;      // и обратно
    }
    
    if (iHighBit > 0){     // выравниваем число по левому краю, поскольку представление в дополнительном коде
